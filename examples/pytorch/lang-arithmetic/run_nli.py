...
from transformers import (
    ...
    HfArgumentParser,
    TrainingArguments,
    set_seed, EvalPrediction, default_data_collator, DataCollatorWithPadding,
)
...
def main():
    ...
    if data_args.task_name == "nala-cub/americas_nli":
        num_labels = 3
    else:
        lang = dataset_languages[0]
        label_list = raw_datasets[lang]["train"].unique("label")
    ...
    model = am.AutoMultiAdapterModelForSequenceClassification.from_pretrained(
    ...
    # TODO FIX change val do test and test to val
    if training_args.do_eval:
        valid_key = "validation" if data_args.task_name != "WillHeld/JamPatoisNLI" else "val"
        if valid_key not in list(raw_datasets.values())[0]:
            raise ValueError("--do_eval requires a validation dataset")

        # For JamPatoisNLI, due to the size, we use the whole dataset as validation.
        if data_args.task_name == "WillHeld/JamPatoisNLI":
            raw_valid_datasets = {n: d["train"] for n, d in
                                  raw_datasets.items()}
        else:
    ...
    if training_args.do_predict:
        ...
        if data_args.task_name == "WillHeld/JamPatoisNLI":
            raw_test_datasets = {n: datasets.concatenate_datasets([d["val"], d["test"]]) for n, d in
                                 raw_datasets.items()}
        else:
    ...
    # Setup adapters
    model = lang_adapter.init_adapters(model, languages, data_args.adapter_path_template, "nli")
            ...
            if trainer.is_world_process_zero():
                with open(output_predict_file, "w") as writer:
                    ...
                    for index, (tp, tl) in enumerate(zip(true_predictions, true_labels)):
                        item = metric.compute(predictions=[tp], references=[tl])["accuracy"]
